		...
		synchronized (MUTEX) {
			...
			if (operations == null) {
				...
				if (!isSet(
						zookeeper)) {
					Assert.fail(
							"Currently HBase tests in GeoWave does not support running on mock instances like Accumulo.\n" + "Please provide a live running instance of zookeeper as zookeeperUrl property.");
					/*
					 ...
					 * (SystemUtils.IS_OS_WINDOWS) { LOGGER.error(
					 * "Windows installation is not yet supported!!!" ); }
